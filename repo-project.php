...
$sqlname = mysqli_query($con, "SELECT `id`, `name` FROM user WHERE id = '$_SESSION[mysesi]'");
...
if (isset($_POST['submit'])) {
  ...
  if (!isset($errorMsg)) {
    $sqlAdd = (mysqli_query($con, "INSERT INTO `project`(`projectID`, `projectName`, `description`, `projectType`, `year`, `department`, `major`, `studentId1`, `studentId2`, `studentId3`, `svID`, `imgDir`, `fileDir`, `abstract`) VALUES (NULL,'$projectName','$description', '$projectType' ,'$year','$department','$major','$sessionId',$studentId2, NULL,'$svID','$cImg','$cPdf' ,'$abstract')"));
    if (!isset($error)) {
      echo "<script>alert ('New record successfully inserted!') </script>";
      if ($department == 'BCS') {
        echo "<script>window.location.assign('bcsproject.php')</script>";
      } else {
        echo "<script>window.location.assign('bitproject.php')</script>";
      }
    } else {
    ...
    <div class="w3-center" style="padding:5% 40px" id="contact">
      ...
      <div style="margin-top:48px" class="w3-center">
        ...
        <form action="" method="POST" enctype="multipart/form-data" class="w3-center" style="width:40%;">
          ...
          <p><strong>Partner Name </strong><select  class="w3-input w3-border" id="studentId2" name="studentId2">
              ...
              ?>
            </select><caption> (Leave this blank if you do not have partner)</caption></p>
